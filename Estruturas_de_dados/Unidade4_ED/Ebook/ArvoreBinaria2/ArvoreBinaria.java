...
public class ArvoreBinaria {
    ...
    private No adicionaNo(No atual, char valor) {
        ...
        if (valor < atual.valor) {
            atual.filhoEsquerda = adicionaNo(atual.filhoEsquerda, valor); // Insere o valor no filho esquerdo do nó corrente
        } else if (valor > atual.valor) { // Valor a ser inserido é maior que o valor do nó corrente
            atual.filhoDireita = adicionaNo(atual.filhoDireita, valor); // Insere o valor no filho direito do nó corrente
    ...
    /*
Essa função chama internamente o método adicionaNo, que é responsável por percorrer a árvore de forma recursiva até encontrar a posição adequada para inserir o novo nó. Esse método retorna o nó atualizado após a inserção do novo valor.

A raiz da árvore é atualizada após a inserção do novo nó utilizando a linha de código raiz = adicionaNo(raiz, valor);, onde raiz é a referência para o primeiro nó da árvore. */
    ...
    private No removeNo(No atual, char valor) {
        ...
        if (valor < atual.valor) {
            ...
            if (atual.filhoEsquerda == null) {
                ...
                atual.filhoDireita = removeNo(atual.filhoDireita, sucessor.valor); // Remove o sucessor da subárvore à direita
                return atual;
            }
        }
    }
    ...
    private No encontraSucessor(No atual) {
        while (atual.filhoEsquerda != null) { // Continua descendo na subárvore à esquerda até encontrar o nó mais à esquerda
    ...
    public void imprimeArvore() {
        imprimeSubArvore(raiz, ""); // Chama o método imprimeSubArvore passando a raiz como o nó atual e uma string vazia como prefixo
    }
    ...
    private void imprimeSubArvore(No atual, String prefixo) {
        ...
        if (!temFilhoEsquerda && !temFilhoDireita) { // Se o nó atual não tiver filhos, imprime o valor do nó com o prefixo
        ...
        if (temFilhoEsquerda) { // Se houver filho à esquerda, chama o método imprimeSubArvore recursivamente passando o filho esquerdo como o nó atual e um prefixo modificado
            imprimeSubArvore(atual.filhoEsquerda, prefixo + (temFilhoDireita ? "|   " : "    "));
        }
    
        if (temFilhoDireita) { // Se houver filho à direita, chama o método imprimeSubArvore recursivamente passando o filho direito como o nó atual e um prefixo modificado
